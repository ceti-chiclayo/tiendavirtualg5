    ...
    <div class="modal-body">
        <div class="form-group row">
            ...
            <div class="col-sm-9 div-input">
                <input type="text" required value="<?= $subcategoria->nombre ?>" name="nombre" id="nombre" class="form-control" />
            </div>
        </div>
        <!-- <div class="form-group row">
...
<script>
    document.getElementById('formulario-editar').addEventListener('submit', function(evento) {
        ...
        actualizar(<?php echo $subcategoria->id ?>);
    });

    function actualizar(id) {
        const formulario = document.getElementById('formulario-editar');
        const datos = new FormData(formulario);
        const ruta = "/subcategorias/update/" + id;
